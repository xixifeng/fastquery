...
public class QueryValidator
{
    ...
    private static void checkQueryByNamed(Map<String, Set<QueryMapper>> mapQueryMapper, String className, Method method)
    {
        ...
        if (queryByNamed != null)
        {
            ...
            else if (StringUtils.EMPTY.equals(tmp.trim()))
            {
            ...
            else
            {
                ...
                if (returnType == Page.class && (countQuery == null || StringUtils.EMPTY.equals(countQuery.trim()))
    ...
    {
        Set<String> ss = TypeUtil.matchesNotrepeat(query, Pattern.compile("(?i)</?where>"));
        ss.forEach(s -> {
    ...
    private static void checkQuery(String query)
    {
        ...
        List<String> mps = TypeUtil.matches(query, RegexCache.EL_REG_PATT);
        for (String mp : mps)
        {
