...
## 微笑表达式
定义: 以 <code>`-</code> 作为开始,以 <code>-`</code> 作为结尾,包裹着若干字符,这样的表达式称之为`微笑表达式`. 例如: <code> `-%${name}%-` </code>.  
作用:  
...
现在有`微笑表达式`了,在模板中,可以配置name实参的模板.假设模板中通过<code>`-:name%-`</code>引用这个实参,那么<code>`-:name%-`</code>将会作为这个实参的模板. 
